...
var_dict["st"] = [["-nb_step", 10], # minimum 1
                  ["-step_size", 1]] # can be set to zero if no surface tracking
...
bool_dict["tracto"] = [[True, "--basis dipy", "--basis mrtrix"],  # fodf input basis
                       ...
                       [True, None, "-test 500"], # Test with only a subpart of the seeding points
...
bool_dict["pft_tracto"] = [[True, "--basis dipy", "--basis mrtrix"],  # fodf input basis
                           [False, "--algo det", "--algo prob"],  # deterministic or probabilistic dMRI tractography
                           [False, "--sh_interp tl", "--sh_interp nn"],
                           [True, "--mask_interp nn", "--mask_interp tl"],
                           [False, "-inv_seed_dir", None],
                           [True, None, "-test 500"], # Test with only a subpart of the seeding points
                           [True, None, "--no_pft"], # Test mask without pft
...
### intersection
bool_dict["intersect"] = [[False, None, "-nuclei hcp/spinal_cord_lps_fx.vtk"],#pft/spinal_lps.vtk"],
                          [False, None, "-nuclei_soft hcp/gray_nuclei_lps_fx.vtk"]]#pft/gray_nuclei_lps.vtk"]]
...
ordered_key = ["lps", "mask", "smooth", "st", "tracto", "pft_tracto", "intersect", "fusion"]
